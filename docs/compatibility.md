...
| GAM-210 loopback host macOS arm64 | .NET 10.0.100 host integration test exercises 127.0.0.1 ephemeral port, denied/paired/revoked requests, Host/Origin and preflight. Node tests exercise the iframe BridgeClient with a mock fetch. **Not a real Figma desktop test.** |
| Figma Design desktop 126.9.10 on macOS 26.6.2 (25G83), GAM-210 | **PASS, owner-observed development-plugin pairing and authenticated request**: the old manifest import rejected `http://127.0.0.1:48931` as an invalid development domain; the corrected `http://localhost:48931` manifest imported. An earlier real-plugin attempt failed with `Failed to execute fetch on Window: Illegal invocation`; after binding the default fetch receiver via `fetch.bind(globalThis)` and rebuilding the plugin, the owner started the sole local host, entered a fresh locally authorized one-time challenge and saw `Paired; 0 registered workspaces (registration is not available yet).` That UI state requires both a successful pair response and an authenticated `/v1/workspaces` response; zero is expected until workspace registration (GAM-214). No local-network permission prompt appeared. Independent curl to unauthenticated `/v1/workspaces` returned 401; synthetic `Origin: null` preflights returned 204. Real Figma preflight packets were **not captured**, so preflight behavior is not independently observed at packet level. Development import does not certify marketplace distribution. |
| Figma Design desktop Windows, GAM-237 | **UNRUN**: no Windows client available here; owner-approved follow-up GAM-237 must repeat actual desktop manifest installation and paired request, recording client/OS versions, permission prompt and preflight observation limits. Spec A19 cross-platform acceptance remains incomplete; macOS success does not establish Windows compatibility. |
| Figma Design desktop on macOS, GAM-208 shell | Owner observed a blank document displaying Selection, Mappings, Preview and changes, and Connection after importing a manually copied `manifest.json`; the original template filename was rejected (`Manifest must be named 'manifest.json'`). Client version was not recorded. A later attempted import of the old build-generated manifest failed in desktop 126.9.10 on macOS due to the IP-literal development allowlist; the corrected localhost manifest subsequently imported and authenticated loopback communication passed after the fetch receiver fix described above. Interactive sample runtime is not implemented. |
